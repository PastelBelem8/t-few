...
do
    ...
    do
        ...
        python -m src.pl_train -c ${model}.json+wmt.json+ia3.json -k load_weight="pretrained_checkpoints/${model}_ia3_finish.pt" save_model=True exp_name=${experiment_name} dataset=${CONFIG_DATASET} data_dir=${CONFIG_DATA_DIR}
    
        echo ------------------------------------------------------------------------------
        echo "Baseline 2: Fine tune TO on training set (w/ IA3): no checkpoint"
        echo ------------------------------------------------------------------------------
        experiment_name=${model}__baseline_no_ckpt_ft_train_ia3
        python -m src.pl_train -c ${model}.json+wmt.json+ia3.json -k save_model=True exp_name=${experiment_name} dataset=${CONFIG_DATASET} data_dir=${CONFIG_DATA_DIR}
        
        echo ------------------------------------------------------------------------------
        echo "Baseline 3: Fine tune TO on training set (w/o IA3)"
        echo ------------------------------------------------------------------------------
        experiment_name=${model}__baseline_no_ckpt_ft_train_no_ia3
        python -m src.pl_train -c ${model}.json+wmt.json+pretrain.json -k save_model=True exp_name=${experiment_name} num_steps=1000 dataset=${CONFIG_DATASET} data_dir=${CONFIG_DATA_DIR}
        ...
        # python -m src.pl_train -c ${model}.json+ia3.json+wmt.json+pretrain.json -k load_weight="pretrained_checkpoints/${model}_ia3_finish.pt" save_model=True exp_name=${experiment_name} num_steps=1000 dataset=${CONFIG_DATASET} data_dir=${CONFIG_DATA_DIR}
        done
done